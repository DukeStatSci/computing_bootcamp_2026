...
The DSS has a Posit Workbench license that will allow you to connect to an instance of RStudio (or Jupyter Lab or VS Code or Positron) in your browser while making use of the computing power of a remote multiprocessor server.
...
![](images/rstudio_login.png){width="85%" fig-align=center .fragment .fade-in-then-out}

![](images/rstudio_sessions.png){width="85%" fig-align=center .fragment .fade-in-then-out}
...
You can also connect to the individual servers directly via SSH (e.g. `ssh netid@hypatia.stat.duke.edu`), the same usage rules apply:

* `hypatia.stat.duke.edu`, `katherinej.stat.duke.edu`, `germain.stat.duke.edu`


## Printers

The department has several printers available to faculty, postdocs, and graduate students (you must be connected to Dukeblue):
