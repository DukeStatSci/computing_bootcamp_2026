...
::: {.aside}
It is highly recommended that you make use of Duke Unlock (passkey) to avoid multi-factor authentication
:::
...
- eduroam (education roaming)
  ...
  - Secure network available at participating universities and research institutions worldwide; log in with your `netid@duke.edu` credentials when visiting other campuses or conference venues

- Visitor

  - Unsecured guest network for campus visitors; does not give access to Duke's private network or resources - as a Duke community member you should be using Dukeblue instead
...
You will need to set up Duo first, but once that is done Duke Unlock (passkey) is the better option for day-to-day use - see instructions at <https://idms-mfa.oit.duke.edu/mfa/home>
...
* The hostnames for these machines are served by Duke's private DNS and only resolve when your device is using Duke's DNS servers, e.g. `rstudio.stat.duke.edu` is `10.138.17.14`.

* Connecting to Dukeblue or the VPN configures all of this automatically, but a manually configured DNS server (e.g. `1.1.1.1` or `8.8.8.8`) will override it and these hostnames will fail to resolve.
